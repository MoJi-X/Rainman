  ...
  <div class="sys-detail">
    <div v-if="!hasSubmit">
      ...
      <a-card title="流程进度" :style="{ marginTop: '12px' }">
        <Step ref="StepDom" :processType="2" :Pid="Pid"></Step>
      </a-card>
      <!-- 加一个校验报错锚点 -->
      ...
      <a-card title="上传立项材料" :border="false" :style="{ marginTop: '12px' }">
        <FileUpload
          ref="FileUploadDom"
      ...
      <a-card title="流程处理" :style="{ marginTop: '12px' }" v-if="operate !== 'view'&&!isView&&!otherView&&(baseInfo.wfNodeCode!='40'||showProcess)">
    ...
    <a-card v-else>
      <ProcessResult
        ...
        :processType="2"
        parentPath="/planning/reviewlist"
      ></ProcessResult>
    </a-card>
...
export default {
  ...
  created() {
    let params = this.$ls.get('reviewDetailId')
  ...
  methods: {
    ...
    selectSys(sysForm) {
      ...
      sysForm.bdFileList.forEach((item,index) => {
        if(sysForm.systemGradingCode!=="project_rank_1"&&index===sysForm.bdFileList.length-1){
          return
        }
        if (
    ...
    async processSubmit() {
      ...
      //提交数据成功后变更hasSubmit
      if ((this.baseInfo.wfNodeCode!='40'||this.showProcess)&&this.processPerson.length === 0) {
        this.$notification.warning({
          message: '请选择流程处理人',
        })
        return
    ...
    goBack() {
      let path = this.isView?'/product/list':'/planning/reviewlist'
      this.$router.push({
        path: path,
      })
    },
