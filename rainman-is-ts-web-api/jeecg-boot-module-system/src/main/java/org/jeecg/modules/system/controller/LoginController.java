...
import org.jeecg.modules.system.util.MiddleHandleUtil;
...
public class LoginController {
    ...
    @Value("${rainman.oauth2.server.url}")
    ...
    public Result<?> getInfo(@RequestParam(name = "userInfo") String userInfo, @RequestParam(name = "wfInstanceId") String wfInstanceId) {
...
//            String test = DESedeCoder.encryptMode("admin");
            ...
            result.getResult().put("wfInstance", wfInstance);
    ...
    @AutoLog(value = "验证accessToken")
    @ApiOperation(value = "验证accessToken", notes = "验证accessToken")
    @PostMapping("/getInfoByToken")
    @ResponseBody
    @ApiImplicitParams({@ApiImplicitParam(name = "access_token", value = "加密用户标识", dataTypeClass = String.class)})
    public Result<?> getInfoByToken(@RequestParam(name = "access_token") String access_token) {
        try {
            String username = MiddleHandleUtil.getByAccessToken(oauth2ServerUrl, access_token);
            if (StringUtils.isEmpty(username)) {
                Result<?> result = new Result<Object>();
                result.error500("无效token");
                return result;
            }
