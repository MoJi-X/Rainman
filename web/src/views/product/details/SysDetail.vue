  ...
  <div class="sys-detail">
    <div v-if="!hasSubmit">
      ...
      <a-card title="流程进度" :style="{ marginTop: '12px' }">
        <Step ref="StepDom" :processType="1" :Pid="Pid"></Step>
      </a-card>
      <a-card
        :title="operate === 'edit' || operate === 'update' ? '系统录入' : '系统项目'"
        :border="false"
        :style="{ marginTop: '12px' }"
      >
        <SysInfo v-if="isView" ref="SysInfoDom" :baseInfo="baseInfo" :canEdit="false"></SysInfo>
        <SysManageList
          v-else
          ref="SysManageListDom"
          :canEdit="baseInfo.wfNodeCode === '10' && !otherView"
          :Pid="Pid"
          @saveData="saveData"
        ></SysManageList>
      </a-card>
      ...
      <a-card
        title="流程处理"
        :style="{ marginTop: '12px' }"
        v-if="operate !== 'view' && !isView && !otherView && (baseInfo.wfNodeCode != '50' || showProcess)"
      >
...
import SysManageList from '@/components/Info/SysManageList'
...
export default {
  ...
  created() {
    let params = this.$ls.get('sysDetailId')
  ...
  methods: {
    ...
    goBack() {
      let path = this.isView ? '/product/list' : '/planning/syslist'
      this.$router.push({
        path: path,
      })
    },
