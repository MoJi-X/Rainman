<template>
  <div class="constr-detail">
    <div v-if="!hasSubmit">
      ...
      <a-card title="上传入网测试材料" :border="false" :style="{ marginTop: '12px' }">
        <FileUpload
          ref="FileUploadDom"
      ...
      <a-card
        ...
        v-if="operate !== 'view' && !isView && !otherView && (baseInfo.wfNodeCode != '70' || showProcess)"
      >
    ...
    <a-card v-else>
      <ProcessResult
        ...
        :Pid="Pid"
        parentPath="/construction/constrlist"
      ></ProcessResult>
    </a-card>
...
export default {
  ...
  computed: {
    ...
    showRiskPoint() {
      return ['30', '40', '50', '60', '70'].indexOf(this.baseInfo.wfNodeCode) !== -1
    },
  },
  created() {
    let params = this.$ls.get('constrDetailId')
  ...
  methods: {
    ...
    async processSubmit() {
      ...
      //提交数据成功后变更hasSubmit
      if ((this.baseInfo.wfNodeCode != '70' || this.showProcess) && this.processPerson.length === 0) {
        this.$notification.warning({
          message: '请选择流程处理人',
        })
        return
    ...
    goBack() {
      let path = this.isView ? '/product/list' : '/construction/constrlist'
      this.$router.push({
        path: path,
      })
    },
