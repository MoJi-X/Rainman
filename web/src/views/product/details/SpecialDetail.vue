  ...
  <div class="special-detail">
    <div v-if="!hasSubmit">
      ...
      <a-card title="流程进度" :style="{ marginTop: '12px' }">
        <Step :processType="3" :Pid="Pid"></Step>
      </a-card>
      <div id="targetTop"></div>
      <a-card title="项目系统信息" :border="false" :style="{ marginTop: '12px' }">
        <SysInfoRecord
          ref="SysInfoRecordDom"
          :baseInfo="baseInfo"
          :canEdit="(operate === 'edit' || operate === 'update')&&!otherView"
        ></SysInfoRecord>
      </a-card>
      ...
      <a-card title="审批意见" v-if="operate === 'check'&&!otherView" :style="{ marginTop: '12px' }">
        <Approval ref="ApprovalDom" @changeResult="(result)=>showProcess=!result"></Approval>
      </a-card>
      <a-card title="流程处理" :style="{ marginTop: '12px' }" v-if="operate !== 'view'&&!otherView&&(baseInfo.wfNodeCode!='50'||showProcess)">
    ...
    <a-card v-else>
      <ProcessResult
        ...
        :Pid="Pid"
        parentPath="/planning/speciallist"
      ></ProcessResult>
    </a-card>
...
import SysInfoRecord from '@/components/Info/SysInfoRecord'
...
export default {
  ...
  created() {
    let params = this.$ls.get('specialDetailId')
  ...
  methods: {
    ...
    async processSubmit() {
      if (!(await this.$refs.SysInfoRecordDom.checkValid())) {
      ...
      //提交数据成功后变更hasSubmit
      if ((this.baseInfo.wfNodeCode!='50'||this.showProcess)&&this.processPerson.length === 0) {
        this.$notification.warning({
          message: '请选择流程处理人',
        })
        return
      ...
      let params = {
        ...
        bdProjectId: this.$refs.SysInfoRecordDom.sysForm.bdProjectId,
