  ...
  <div class="disposal-detail">
    <div v-if="!hasSubmit">
      ...
      <a-card title="流程进度" :style="{ marginTop: '12px' }">
        <Step :processType="9" :Pid="Pid"></Step>
      </a-card>
      ...
      <a-card title="流程处理" :style="{ marginTop: '12px' }" v-if="operate !== 'view'&& !otherView&&(baseInfo.wfNodeCode!='20'||showProcess)">
    ...
    <a-card v-else>
      <ProcessResult
        ...
        :Pid="Pid"
        parentPath="/running/disposallist"
      ></ProcessResult>
    </a-card>
...
export default {
  ...
  created() {
    let params = this.$ls.get('disposalDetailId')
  ...
  methods: {
    ...
    async processSubmit() {
      ...
      //提交数据成功后变更hasSubmit
      if ((this.baseInfo.wfNodeCode!='20'||this.showProcess)&&this.processPerson.length === 0) {
        this.$notification.warning({
          message: '请选择流程处理人',
        })
        return
